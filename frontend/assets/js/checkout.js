...
(function ($) {
	...
	$(document).on("change", "#states", function (e) {
		$.get(_lsdcommerce.plugin_url + 'assets/cache/' + $('#country').val() + '-cities.json', function (data, status) {
	...
	$(document).on("click", ".lsdcommerce-create-order", function (e) {
		...
		if (customer && shipping && products) {
			...
			lsdc_checkout_nextslide(0);
			checkout.find('#checkout-alert p').text(_lsdcommerce.translation.data_incorrect);
		}
