/**
 * Bertanggung jawab untuk fungsi-fungsi di detail produk
 * seperti ::
 * - Menambah produk ke Keranjang
 * - Menambah dan Mengurang Produk dari Keranjang
 * -
 * 
 * Responsible for function in product details
 * 
 * @since 0.3.0
 */
(function ($) {
	...
	$(document).ready(function () {
		...
		if (total == undefined) {
			...
			totalText = _lsdcommerce.translation.cart_empty; // Empty
		} else {
	...
	$(document).on('click', '.lsdc-add-to-cart', function (e) {
		e.preventDefault();
	...
	$(document).on('click', '.minus', function (e) {
		...
		if (total == undefined) {
			...
			lsdcommerce_single_cartmanager(total.total_qty, _lsdcommerce.translation.cart_empty);
			controlQty.hide();
		} else {
			lsdcommerce_single_cartmanager(total.total_qty, lsdcommerce_currency_format(true, total.total_price));
		}
